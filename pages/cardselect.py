...
def cards_select(root, type):
    ...
    menu = OptionMenu(root, menu_var, *menu_options, command=export_menu).pack(pady=10)
    spacer_2 = ttk.Label(root, text="|", foreground="#191919").pack(pady=10)

    start_button = ttk.Button(root, text="Start A Training Session", command=lambda: export_menu(menu_var)).pack()